#!/usr/bin/env bash
#
# prepare-pr-validation <feature-branch> [--base <base-branch>]
#
...
usage() {
  echo "Uso: prepare-pr-validation <feature-branch> [--base <base-branch>]" >&2
  exit 1
}
...
# Se já existir um worktree para este PR, recria do zero (garante estado limpo)
...
git worktree add --detach "$WORKTREE_PATH" "origin/${FEATURE_BRANCH}"
...
echo "${FEATURE_BRANCH}|${BASE_BRANCH}|${WORKTREE_PATH}|${MERGE_BASE}" >> "$STATE_DIR/active-worktrees"
